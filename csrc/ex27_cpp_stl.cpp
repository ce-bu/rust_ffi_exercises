...
{
    if (!s || !out_len) return CPP_STK_ERR_NULL;
    try {
        /* Peek first — don't remove until we know the buffer fits. */
        const std::string &top = s->peek();
        *out_len = top.size();
        if (!out_buf || buf_len < top.size() + 1) return CPP_STK_ERR_BUF;
...
{
    if (!s || !out_ptr || !out_len) return CPP_STK_ERR_NULL;
    try {
...
int32_t cpp_stk_push_many(CppStringStack *s,
                           ...
                           size_t count)
{
    if (!s || (!strings && count > 0)) return CPP_STK_ERR_NULL;
    try {
